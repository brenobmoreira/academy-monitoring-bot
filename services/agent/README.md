...
| `bot.py` | Instruction, one ADK run per message, `MAX_LLM_CALLS` budget; the model's text is escaped |
| `telegram.py` | `sendMessage` (optional `parse_mode=HTML`, `reply_markup`, reply-to; returns the sent Message), `getUpdates` |
| `handler.py` | Allowlist, `/start`, run the bot, reply as HTML in as many messages as needed; never raises |
